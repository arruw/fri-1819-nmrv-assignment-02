...
total = size(t_bins, 2)*size(t_eps, 2)*size(t_alphas, 2)*size(t_lambdas, 2)*size(t_sequences, 2);

for sequence = t_sequences
for bins = t_bins
for eps = t_eps
for alpha = t_alphas
for lambda = t_lambdas
       
    if mod(ri, 5) == 0
        T = table(r_id', r_sequence', r_bins', r_eps', r_alpha', r_lambda', r_n_frames', r_n_failures', r_time' , r_error',...
            'VariableNames',{'id', 'Sequence', 'n_bins', 'eps', 'alpha', 'lambda', 'n_frames', 'n_failures', 'time', 'error'});
